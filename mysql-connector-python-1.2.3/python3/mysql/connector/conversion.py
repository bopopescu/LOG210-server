...
class HexLiteral(str):
    ...
    def __new__(cls, str_, charset='utf8'):
        hexed = ["{0:x}".format(i) for i in str_.encode(charset)]
...
class MySQLConverter(MySQLConverterBase):
    ...
    def escape(self, value):
        ...
        if value is None:
            return value
        elif isinstance(value, (int, float, Decimal, HexLiteral)):
            return value
    ...
    def quote(self, buf):
        ...
        """
        if isinstance(buf, (int, float, Decimal, HexLiteral)):
    ...
    def _timedelta_to_mysql(self, value):
        ...
        if value.microseconds:
            return fmt.format(hours, mins, secs, mcs).encode('ascii')
        return fmt.format(hours, mins, secs).encode('ascii')
    ...
    def _TIME_to_python(self, value, dsc=None):  # pylint: disable=C0103
        ...
        try:
            ...
            raise ValueError(
                "Could not convert {} to python datetime.timedelta".format(
                 value))
        else:
            return time_val
    ...
    def _STRING_to_python(self, value, dsc=None):  # pylint: disable=C0103
        ...
        if isinstance(value, bytes) and self.use_unicode:
